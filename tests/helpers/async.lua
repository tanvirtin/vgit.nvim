...
return function(fns)
  local wrapped = {}
  if fns.it then wrapped.it = function(name, fn)
    fns.it(name, function()
      run(fn)
    end)
  end end
  if fns.before_each then
    wrapped.before_each = function(fn)
      fns.before_each(function()
        run(fn)
      end)
    end
  end
  if fns.after_each then
    wrapped.after_each = function(fn)
      fns.after_each(function()
        run(fn)
      end)
    end
  end
