...
describe('fs:', function()
    ...
    describe('tracked_filename', function()
        ...
        it('should throw error on invalid argument types', function()
            ...
            assert.has_error(function()
                fs.tracked_filename(1, {})
            end)
            assert.has_error(function()
                fs.tracked_filename(nil, {})
            end)
            assert.has_error(function()
                fs.tracked_filename(function() end, {})
            end)
            assert.has_error(function()
                fs.tracked_filename('', nil)
            end)
            assert.has_error(function()
                fs.tracked_filename('', { foo = 'bar' })
            end)
            assert.has_error(function()
                fs.tracked_filename('', true)
            end)
        ...
        it('should retrieve the most relevant path from a list of paths', function()
            eq(fs.tracked_filename('foo', { 'bar/foo', 'foo/bar', 'bar/baz' }), 'bar/foo')
            eq(fs.tracked_filename('foo', { 'bar/baz/foo', 'bar/foo', 'foo/bar' }), 'bar/baz/foo')
        end)

        it('should always retrieve the item which follows the longest pattern first', function()
            eq(fs.tracked_filename('foo', { 'bar/foo', 'bar/baz/foo', 'foo/bar' }), 'bar/baz/foo')
        end)

        it('should return nil if an empty list is provided', function()
            local result = fs.tracked_filename('foo', {})
            eq(result, nil)
        end)
