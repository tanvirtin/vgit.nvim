...
describe('TreeComponent - Core Functionality', function()
  ...
  describe('derive_entry_hl', function()
    ...
    it('should return GitSignsAdd for staged added files', function()
      ...
      local hl = component:derive_entry_hl(status)
  ...
  describe('create_node - file nodes', function()
    ...
    it('should create virtual_text before structure', function()
      local entry = create_file_entry()
      local node = component:create_node(entry)
      assert.is_not_nil(node.virtual_text)
      assert.is_not_nil(node.virtual_text.before)
      assert.are.equal('M ', node.virtual_text.before.text)
    end)
  ...
  describe('create_node - folder nodes', function()
    ...
    it('should have icon_before function for fold state', function()
      local entry = create_folder_entry()
      local node = component:create_node(entry)
      assert.is_function(node.icon_before)
    end)

    it('should return open icon when folder is open', function()
      local entry = create_folder_entry()
      local node = component:create_node(entry)
      node.open = true
      local icon_info = node.icon_before(node)
      assert.is_not_nil(icon_info.icon)
    end)

    it('should return closed icon when folder is closed', function()
      local entry = create_folder_entry()
      local node = component:create_node(entry)
      node.open = false
      local icon_info = node.icon_before(node)
