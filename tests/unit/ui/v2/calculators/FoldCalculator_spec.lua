...
describe('FoldCalculator', function()
  ...
  describe('calculate_folds', function()
    ...
    it('should create fold before first mark when sufficient space', function()
      ...
      -- Should have a fold from focus+1 to mark.top-focus-1
      assert.is_true(#folds > 0)
      local first_fold = folds[1]
      assert.equals(8, first_fold.top) -- num_focus_lines (7) + 1
    ...
    it('should create fold after last mark when sufficient space', function()
      ...
      -- Should have a fold after the last mark
      assert.is_true(#folds > 0)
    ...
    it('should create folds between consecutive marks', function()
      ...
      -- Should have at least one fold between the marks
    ...
    it('should respect custom num_focus_lines parameter', function()
      local marks = {
        { top = 50, bot = 60 },
      }
      local line_count = 200
      local num_focus_lines = 10

      local folds = calculator:calculate_folds(marks, line_count, num_focus_lines)

      assert.is_not_nil(folds)
      if #folds > 0 then
        local first_fold = folds[1]
        assert.equals(11, first_fold.top) -- num_focus_lines (10) + 1
      end
    end)
    ...
    it('should only create folds with minimum 10 lines', function()
      ...
      -- All folds should be at least 10 lines
      for _, fold in ipairs(folds) do
        local fold_size = fold.bot - fold.top + 1
        assert.is_true(fold_size >= 10, 'Fold size ' .. fold_size .. ' is less than 10')
      end
    end)
    ...
    it('should not create overlapping folds with marks', function()
      ...
      -- Verify no fold overlaps with any mark
      for _, fold in ipairs(folds) do
        for _, mark in ipairs(marks) do
          local overlaps = not (fold.bot < mark.top or fold.top > mark.bot)
          assert.is_false(overlaps, 'Fold overlaps with mark')
        end
      end
    end)

    it('should handle edge case with marks at file boundaries', function()
      local marks = {
        { top = 1, bot = 10 },
        { top = 190, bot = 200 },
      }
      ...
      -- Should create fold in the middle
      assert.is_true(#folds > 0)
    end)
