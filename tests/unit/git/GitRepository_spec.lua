...
describe('GitRepository', function()
  ...
  describe('status', function()
    it('returns empty status for clean repository', function()
      ...
      assert(#status == 0 or status.entries ~= nil, 'should be empty or have entries table')
    end)
    ...
    it('returns status for specific file', function()
      ...
      local opened_repo = GitRepository.open(test_repo.dir)
      local status, err = opened_repo:status({ filename = 'test.txt' })
      assert(err == nil, 'should not return error')
  ...
  describe('index', function()
    ...
    it('creates fresh index on each call', function()
      local opened_repo = GitRepository.open(test_repo.dir)
      local index1, _ = opened_repo:index()
      local index2, _ = opened_repo:index()
      assert(index1 ~= index2, 'should return different instances')
      assert(index1 ~= nil and index2 ~= nil, 'both should be valid')
    end)
  end)
  ...
  describe('refs', function()
    ...
    it('creates fresh refs on each call', function()
      local opened_repo = GitRepository.open(test_repo.dir)
      local refs1, _ = opened_repo:refs()
      local refs2, _ = opened_repo:refs()
      assert(refs1 ~= refs2, 'should return different instances')
      assert(refs1 ~= nil and refs2 ~= nil, 'both should be valid')
    end)
  end)
