...
local function make_paddings(rows, column_labels, column_spacing, max_column_len)
    local padding = {}
    for i = 1, #rows do
        local items = rows[i]
...
local function make_heading(paddings, column_labels, column_spacing, max_column_len)
...
function TableComponent:set_lines(lines, force)
    ...
    local column_header = make_heading(paddings, header, column_spacing, max_column_len)
