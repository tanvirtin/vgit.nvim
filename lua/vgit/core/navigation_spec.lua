...
describe('navigation:', function()
  ...
  describe('up', function()
    it('should select the last mark when the cursor is above the first mark', function()
      for i = 1, marks[1].top do
        vim.api.nvim_win_get_cursor.returns({ i, 0 })
        eq(navigation.up(window, marks), #marks)
      end
    end)
    it('should select the second last mark if the cursor is inside the last mark', function()
      for i = marks[#marks].top, marks[#marks - 1].bot, -1 do
        vim.api.nvim_win_get_cursor.returns({ i, 0 })
        eq(navigation.up(window, marks), #marks - 1)
      end
    end)
    it('should jump to the top of the mark if the cursor is in the middle of the mark', function()
      local selected = 2
    ...
    it('should jump to the top of the mark if the cursor is in the end of the mark', function()
      local selected = 2
      local mark = marks[selected]
    ...
    it('should jump to the end of the previous mark if the cursor is in the top of the current mark', function()
      local selected = 2
      local mark = marks[selected]
  ...
  describe('down', function()
    it('should select the first mark when the cursor on the end of last mark', function()
      local mark = marks[#marks]
      vim.api.nvim_win_get_cursor.returns({ mark.bot, 0 })
      eq(navigation.down(window, marks), 1)
    end)
    it('should select the second mark if the user is on the end of the first mark', function()
      vim.api.nvim_win_get_cursor.returns({ marks[2].top, 0 })
      eq(navigation.down(window, marks), 2)
    end)
    it('should jump to the end of the mark if the cursor is in the middle of the mark', function()
      local selected = 1
      local mark = marks[selected]
    ...
    it('should jump to the end of the mark if the cursor is on top of the mark', function()
      local selected = 1
      local mark = marks[selected]
      vim.api.nvim_win_get_cursor.returns({ mark.top, 0 })
      eq(navigation.down(window, marks), selected)
