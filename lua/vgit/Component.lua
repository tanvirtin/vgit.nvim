...
function Component:get_virtual_line_nr_buf()
    return self:get_virtual_line_nr() and self:get_virtual_line_nr():get_buf() or nil
end

function Component:get_virtual_line_nr_win_id()
    return self:get_virtual_line_nr() and self:get_virtual_line_nr():get_win_id() or nil
end
...
function Component:set_virtual_line_nr_lines(lines, hls)
    ...
    self:set_virtual_line_nr(
        VirtualLineNrDecorator:new(self.config:get('virtual_line_nr'), self.config:get('window_props'), self:get_buf())
    )
...
function Component:set_centered_animated_text(frame_rate, frames, force, callback)
...
function Component:set_centered_text(text, in_animation, force)
    ...
    lines[math.ceil(height / 2)] = string.rep(' ', dimensions.calculate_text_center(text, width)) .. text
