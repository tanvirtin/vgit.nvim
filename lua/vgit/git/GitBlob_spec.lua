...
describe('GitBlob', function()
  ...
  describe('integration', function()
    ...
    describe('content', function()
      it('should error because git_show.content is undefined', function()
        local blob = GitBlob(repo, 'file.txt')
        local ok, err = pcall(function()
          return blob:content()
        end)
        -- git_show only exports lines(), not content()
        assert.is_false(ok)
      end)
    end)

    describe('hash', function()
      it('should error because git_show.hash is undefined', function()
        local blob = GitBlob(repo, 'file.txt')
        local ok, err = pcall(function()
          return blob:hash()
        end)
        -- git_show only exports lines(), not hash()
        assert.is_false(ok)
      end)
    end)
