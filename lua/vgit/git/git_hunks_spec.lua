...
describe('git_hunks:', function()
  describe('live', function()
    ...
    it('should detect added lines at the beginning', function()
      ...
      eq(hunks[1].diff, { '+a' })
      eq(hunks[1].stat, { added = 1, removed = 0 })
    end)
    ...
    it('should detect multiple added lines', function()
      ...
      eq(hunks[1].diff, { '+b', '+c' })
      eq(hunks[1].stat, { added = 2, removed = 0 })
    end)
    ...
    it('should detect multiple removed lines', function()
      ...
      assert.are.equal(hunks[1].bot, 1)
      eq(hunks[1].diff, { '-b', '-c' })
      eq(hunks[1].stat, { added = 0, removed = 2 })
    end)
    ...
    it('should detect changed lines', function()
      ...
      assert.are.equal(hunks[1].bot, 2)
      eq(hunks[1].diff, { '-b', '+x' })
      eq(hunks[1].stat, { added = 1, removed = 1 })
    end)
    ...
    it('should detect multiple changed lines', function()
      ...
      assert.are.equal(hunks[1].top, 2)
      assert.are.equal(hunks[1].bot, 3)
      eq(hunks[1].diff, { '-b', '-c', '+x', '+y' })
      eq(hunks[1].stat, { added = 2, removed = 2 })
    end)
    ...
    it('should detect asymmetric changes (more added than removed)', function()
      ...
      assert.are.equal(hunks[1].top, 2)
      assert.are.equal(hunks[1].bot, 4)
      eq(hunks[1].diff, { '-b', '+x', '+y', '+z' })
      eq(hunks[1].stat, { added = 3, removed = 1 })
    end)
    ...
    it('should detect asymmetric changes (more removed than added)', function()
      ...
      assert.are.equal(hunks[1].top, 2)
      assert.are.equal(hunks[1].bot, 2)
      eq(hunks[1].diff, { '-b', '-c', '-d', '+x' })
      eq(hunks[1].stat, { added = 1, removed = 3 })
    end)
    ...
    it('should detect multiple separate hunks', function()
      ...
      eq(hunks[2].diff, { '-e', '+y' })
      eq(hunks[2].stat, { added = 1, removed = 1 })
    end)
    ...
    it('should detect new file (all lines added)', function()
      ...
      assert.are.equal(hunks[1].bot, 3)
      eq(hunks[1].diff, { '+a', '+b', '+c' })
      eq(hunks[1].stat, { added = 3, removed = 0 })
    end)
  ...
  describe('custom', function()
    ...
    it('should handle empty file for deleted', function()
      local lines = {}
    ...
    it('should return hunk with nil type when opts has neither untracked nor deleted', function()
      local lines = { 'a', 'b' }

      local hunks = git_hunks.custom(lines, {})
  ...
  describe('list', function()
    ...
    it('should return error when filenames has wrong count', function()
      local result, err = git_hunks.list('/tmp', { filenames = { 'one.txt' } })
      assert.is_nil(result)
      assert.is_not_nil(err)
      assert.are.same({ 'incorrect number of files provided' }, err)
    end)

    it('should return error when filenames has three files', function()
      local result, err = git_hunks.list('/tmp', { filenames = { 'a.txt', 'b.txt', 'c.txt' } })
      assert.is_nil(result)
      assert.is_not_nil(err)
      assert.are.same({ 'incorrect number of files provided' }, err)
    end)
