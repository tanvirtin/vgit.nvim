...
local Query = require('vgit.features.screens.StagedDiffHunkScreen.Query')

local StagedDiffHunkScreen = Feature:extend()

function StagedDiffHunkScreen:constructor()
  local scene = Scene()
  local query = Query()

  return {
    name = 'Staged Diff Hunk Screen',
    scene = scene,
    query = query,
    layout_type = nil,
...
function StagedDiffHunkScreen:trigger_keypress(key, ...)
  self.scene:trigger_keypress(key, ...)

  return self
end

function StagedDiffHunkScreen:show()
...
function StagedDiffHunkScreen:destroy()
  self.scene:destroy()
