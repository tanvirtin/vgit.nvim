...
do
  if ngx then
    ...
    do
      ...
      if not ok then new_tab = function(narr, nrec)
        return {}
      end end
    end
    ...
    local function hex_to_i(s)
      ...
      if n == -1 then error('could not convert hex to number') end
    ...
    local function factory(namespace, hash_fn)
      if not _M.is_valid(namespace) then return nil, 'namespace must be a valid UUID' end

      local i = 0
      local iter, err = gmatch(namespace, [[([\da-f][\da-f])]])
      if not iter then return nil, 'could not create iter: ' .. err end
